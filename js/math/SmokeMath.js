/**
 * SmokeMath — Volumetric Particle System with Perlin Noise
 * Ported and adapted for Harmonia.
 * Simulates rising smoke puffs with stochastic turbulence.
 */
export class SmokeMath {
    constructor() {
        this.particles = [];
        this.maxParticles = 500;
        this.energy = 0;
        
        // Simplex Noise (simplified for Harmonia)
        this.perm = new Uint8Array(512);
        const p = [151,160,137,91,90,15,131,13,201,95,96,53,194,233,7,225,140,36,103,30,69,142,8,99,37,240,21,10,23,190,6,148,247,120,234,75,0,26,197,62,94,252,219,203,117,35,11,32,57,177,33,88,237,149,56,87,174,20,125,136,171,168,68,175,74,165,71,134,139,48,27,166,77,146,158,231,83,111,229,122,60,211,133,230,220,105,92,41,55,46,245,40,244,102,143,54,65,25,63,161,1,216,80,73,209,76,132,187,208,89,18,169,200,196,135,130,116,188,159,86,164,100,109,198,173,186,3,64,52,217,226,250,124,123,5,202,38,147,118,126,255,82,85,212,207,206,59,227,47,16,58,17,182,189,28,42,223,183,170,213,119,248,152,2,44,154,163,70,221,153,101,155,167,43,172,9,129,22,39,253,19,98,108,110,79,113,224,232,178,185,112,104,218,246,97,228,251,34,242,193,238,210,144,12,191,179,162,241,81,51,145,235,249,14,239,107,49,192,214,31,181,199,106,157,184,84,204,176,115,121,50,45,127,4,150,254,138,236,205,93,222,114,67,29,24,72,243,141,128,195,78,66,215,61,156,180];
        for (let i = 0; i < 256; i++) this.perm[i] = this.perm[i + 256] = p[i];
    }

    // Standard API used by new Mode renderers
    addPulse(normalizedX, energy) {
        this.addPuff(normalizedX, 0.8, Math.floor(5 + energy * 15));
        this.energy = Math.min(1.0, this.energy + energy);
    }

    update(dt, complexity) {
        this.energy *= 0.94;
        this.step(dt, 0.5, complexity, 1);
    }

    getAudioModulation() {
        return {
            filterMod: Math.min(1, this.energy * 2.5),
            detuneMod: Math.min(1, this.particles.length * 0.005),
            harmonics: Math.min(1, this.energy * 1.5)
        };
    }

    noise(x, y, z) {
        const X = Math.floor(x) & 255, Y = Math.floor(y) & 255, Z = Math.floor(z) & 255;
        x -= Math.floor(x); y -= Math.floor(y); z -= Math.floor(z);
        const fade = t => t * t * t * (t * (t * 6 - 15) + 10);
        const lerp = (a, b, t) => a + (b - a) * t;
        const grad = (hash, x, y, z) => {
            const h = hash & 15;
            const u = h < 8 ? x : y;
            const v = h < 4 ? y : h === 12 || h === 14 ? x : z;
            return ((h & 1) === 0 ? u : -u) + ((h & 2) === 0 ? v : -v);
        };
        const u = fade(x), v = fade(y), w = fade(z);
        const A = this.perm[X] + Y, AA = this.perm[A] + Z, AB = this.perm[A + 1] + Z;
        const B = this.perm[X + 1] + Y, BA = this.perm[B] + Z, BB = this.perm[B + 1] + Z;
        return lerp(lerp(lerp(grad(this.perm[AA], x, y, z), grad(this.perm[BA], x - 1, y, z), u), lerp(grad(this.perm[AB], x, y - 1, z), grad(this.perm[BB], x - 1, y - 1, z), u), v), lerp(lerp(grad(this.perm[AA + 1], x, y, z - 1), grad(this.perm[BA + 1], x - 1, y, z - 1), u), lerp(grad(this.perm[AB + 1], x, y - 1, z - 1), grad(this.perm[BB + 1], x - 1, y - 1, z - 1), u), v), w);
    }

    addPuff(x, y, amount) {
        for (let i = 0; i < amount; i++) {
            this.particles.push({
                x: x + (Math.random() - 0.5) * 0.05,
                y: y,
                vx: (Math.random() - 0.5) * 0.01,
                vy: -Math.random() * 0.02,
                size: 0.1 + Math.random() * 0.2,
                life: 1,
                decay: 0.005 + Math.random() * 0.01,
                rotation: Math.random() * Math.PI * 2,
                rotSpeed: (Math.random() - 0.5) * 0.1
            });
        }
        if (this.particles.length > this.maxParticles) {
            this.particles.splice(0, this.particles.length - this.maxParticles);
        }
    }

    step(dt, intensity, complexity, speed) {
        const time = Date.now() * 0.001;
        this.particles = this.particles.filter(pt => {
            pt.life -= pt.decay * (0.5 + Math.random() * 0.5);
            if (pt.life <= 0) return false;

            // Turbulence
            const turb = this.noise(pt.x * 10, pt.y * 10, time * speed);
            pt.vx += (turb - 0.5) * 0.001 * complexity;
            pt.vy -= 0.001 * intensity;
            
            pt.x += pt.vx;
            pt.y += pt.vy;
            pt.size += 0.002;
            pt.rotation += pt.rotSpeed;
            return true;
        });
    }

    clear() {
        this.particles = [];
    }
}
